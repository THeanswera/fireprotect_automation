...
- **CONFIRMED (узкая область):** вопрос расчётной длины и закрепления решён по
  первичному описанию алгоритма. В поставляемом с RX3 проекте НД
  (`rx3/doc/pages/pr.pdf`), раздел 4, подраздел «Изгибаемый стержень в одной из
  главных плоскостей» (формулы 3 и 4), критическая температура определяется по
  моменту, моменту сопротивления, поперечной силе, моменту инерции и
  минимальной толщине; расчётная длина и вид опирания входят в подраздел
  «Сжатый стержень» (формулы 8–13, λ = L / i_min). Статус
  `NOT_USED_BY_THIS_ALGORITHM` записывается вместе со ссылкой на документ и его
  SHA-256. Прежняя формулировка `NOT_APPLICABLE_FOR_THIS_RECORD`, опиравшаяся на
  `N = 0` и на прежнее поведение программы, снята как недоказанная.
- **UNKNOWN:** нормативная применимость проекта НД для Rx3 (не утверждённый
  документ), тождество расчётного алгоритма программы и формул проекта НД, а
  также инженерная применимость методики к конкретной конструкции не
  подтверждены; выбор режима и переключателя «прочность/устойчивость» остаётся
  инженерным решением. Перенос данных не считается подтверждением
  инженерного расчёта (`experience_scope`).
- **CONFIRMED (узкая область):** первый сквозной учебный результат на одной
  строке РСУ: подготовленный `generated.rx38` (поля 50/92) рассчитан в RX3,
  сохранён как `calculated.rx38` и проверен `validate-rx3-result`. Пересчёт
  доказан (`byte_identical=false`, result-поля 44/54 изменились), 16 нецелевых
  записей не изменены, подтверждение инженера зафиксировано
  (`RX3_RESULT_ANALYSED`, `ENGINEER_CONFIRMED`). Критическая температура
  `728.806265661724 °C`, собственный предел `15.6833333333333 мин` при
  требуемом R15. Это результат учебного опыта, а не заключение по объекту:
  нормативная применимость, толщина и расход огнезащиты не подтверждены.
- **CONFIRMED:** RX3 сохраняет введённый момент с точностью окна ввода
  (`6,1781895` → `6,18`), а сохранённая копия field78 обновляется до того же
  значения; точное перенесённое значение остаётся в `generated.rx38`.
- **CONFIRMED:** `export-lira-bar-review` создаёт новую обзорную книгу
  ...
  источники, роли решений, блокеры) и заполняет лист «Результат RX3» из отчёта
  `validate-rx3-result` вместе со статусом проверки, подтверждением и SHA-256
  рассчитанного файла. До появления отчёта ячейки результата остаются пустыми;
  это не расчётная книга ОБМ.
