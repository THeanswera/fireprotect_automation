...
Вопрос расчётной длины разрешён по подтверждённым фактам, а не отложен на
визуальную сверку неизвестного с неизвестным: выбранная строка не содержит
осевого усилия (`N = 0`), запись классифицируется как изгибаемый стержень в
одной из главных плоскостей, а контролируемый опыт `LIRA-RX3-22P-XX-MAGNITUDE`
на этой же записи Б2 выполнен при незаполненном `field48` и `field51='0'`,
причём RX3 не потребовал их заполнения и не изменил их. Поэтому статус —
`NOT_APPLICABLE_FOR_THIS_RECORD`; геометрические 3 м расчётной длиной не
назначаются, поля 48/51/141 не интерпретируются. Для сжатых и
сжато-изогнутых элементов вывод не действует.
