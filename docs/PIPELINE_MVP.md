...
ЛИРА выводит длинную таблицу усилий страницами, поэтому `--forces` принимает
несколько файлов: `--forces <стр1.xls> <стр2.xls> ...`. Каждая страница остаётся
отдельным источником со своим путём и SHA-256; страница, повторяющая уже
прочитанную пару (элемент, сечение, загружение), отклоняется, а не сливается и не
перезаписывается. Пустая страница тоже отклоняется. Отчёт перечисляет каждую
страницу (`force_pages`) и остатки по компонентам. Готовый `--output-dir`
(evidence-каталог) по-прежнему принимает ровно один файл усилий: страничная
выгрузка в этом формате пока не представима, поэтому каталог не создаётся.
...
## Одна команда подготовки одного опыта

`prepare-lira-bar-run` сводит подготовку к четырём входным значениям: путь к
существующему проверенному пакету, `row_id` опубликованной строки РСУ, ссылка
на постановку опыта и новый каталог вывода. Пути исходных XLS, листы, строки
заголовков, коэффициенты, геометрия, точные десятичные значения и хеши
берутся из повторно прочитанных источников, а не вводятся человеком.
...
Поведение: `--dry-run` выполняет все проверки и ничего не пишет; расхождение
хешей любого записанного источника даёт `SOURCE_DRIFT_DETECTED` и требует
явного `--accept-current-sources` (принятое расхождение попадает в манифест);
повторный запуск с теми же входами даёт `RUN_ALREADY_PREPARED` и не дублирует
каталог; каталог с чужим манифестом или с конфликтующими файлами не
перезаписывается. Цепочка КЭ, её концевые узлы и единый профиль выводятся из
структуры модели: равенство марок не используется как основание объединения,
ветвление, разрыв, обратное направление КЭ и ненулевой поворот отклоняются.

Декларация опыта, которую строит команда, всегда `DRAFT_UNSIGNED`: поля
`confirmed_by`/`selected_by` обязаны быть `null`, а каждое решение несёт роль
(`USER_STATEMENT`, `SOURCE_DOCUMENT`, `PACKAGE_EVIDENCE`,
`ASSISTANT_SELECTION`). Подписанная `ENGINEER_SIGNED` декларация заполняется
человеком и роли в ней не ведутся. Ни один из этих путей не открывает
production.

## Учебный опыт Б2: подготовка расчётного файла

Для уже подтверждённой узкой области (`Б2 / 22П / ГОСТ 8240-97 / L=3.00 m /
rotation 0 / one-plane X-X`) существует VALIDATION-only шаг, который
переносит значения проверенной строки в поля 50 и 92 совместимой записи:
...
Значения, единицы и допустимая конвенция выводятся заново из перечитанных
исходных XLS и привязанного evidence (`read_verified_bar_run`), а не из
сохранённого JSON. Сохранённый `experiment/experiment_input.json` читается
только для обнаружения подмены: проверяются полнота, уникальность и имена
шести компонентов, их значения, единицы и конвенция, а также идентичность
стержня, концевых узлов, длины и профиля по отношению к пересчитанным данным.
Декларация обязана остаться привязанной к записанной версии evidence
(`linked_manifest_sha256`), иначе подготовка отклоняется.

Запись шаблона принимается не по совпадению марки, сортамента и длины:
используется существующая подтверждённая классификация напряжённого состояния
(изгибаемый стержень в одной из главных плоскостей), сверяются требуемый
предел огнестойкости и режим пожара, а сам файл шаблона должен совпадать с
контролируемым SHA-256. Смысл `unknown`-полей не выводится и scope не
расширяется.

Он отказывает при другом scope, другом преобразовании (кроме `MAGNITUDE`),
блокированной передаче, неоднозначной целевой записи, существующем каталоге
вывода и режиме `PRODUCTION`. После записи файл перечитывается: доказывается,
что изменилась только целевая запись и только разрешённые поля, а остальные
записи побайтово идентичны шаблону.

Вопрос расчётной длины решён по первичному описанию алгоритма, а не по
поведению программы. В поставляемом с RX3 проекте НД
(`rx3/doc/pages/pr.pdf`, «Проект НД для Rx3. Инструкция. Определение пределов
огнестойкости стальных строительных конструкций с огнезащитным покрытием»),
раздел 4 «Определение критической температуры»: в подразделе «Изгибаемый
стержень в одной из главных плоскостей» (формулы 3 и 4) критическая
температура определяется по максимальному моменту, моменту сопротивления,
поперечной силе, моменту инерции, статическому моменту полусечения и
минимальной толщине; расчётная длина и вид опирания в эти формулы не входят и
применяются в подразделе «Сжатый стержень» (формулы 8–13, λ = L / i_min, где
«L — расчетная длина стержня, в зависимости от вида опирания и длины стержня
L0»). Поэтому статус — `NOT_USED_BY_THIS_ALGORITHM` с точной ссылкой на
источник и SHA-256 файла.

Прежняя формулировка `NOT_APPLICABLE_FOR_THIS_RECORD`, опиравшаяся на `N = 0` и
на прежний удачный расчёт RX3 при незаполненных полях, снята: ни то, ни другое
не является свидетельством об алгоритме. Эмпирические наблюдения сохранены в
отчёте отдельным полем `empirical_observations_not_used_as_evidence` и как
основание не используются.

Отдельно фиксируются три разные области: интерфейс (справка показывает вид
опирания и коэффициент расчётной длины как отдельные элементы и не
перечисляет, для каких режимов они требуются), расчётный алгоритм (формулы
проекта НД) и инженерная применимость. Последняя остаётся за инженером: проект
НД не является утверждённым нормативным документом, тождество алгоритма
программы и формул не верифицировано, выбор режима и переключателя
«прочность/устойчивость» — инженерное решение, а Приложение 4 требует
постоянного поперечного сечения по длине. Пакет не выдаёт перенос данных за
подтверждение инженерного расчёта: это разделено в блоке `experience_scope`.

Команда продолжения (`validation_command` в манифесте подготовки) не содержит
`--gui-evidence ENGINEER_CONFIRMED`: сохранение `calculated.rx38` само по себе
не является человеческим подтверждением, и подставлять его автоматически
запрещено.

Обзорный Excel учебного опыта создаётся отдельной командой
`export-lira-bar-review` и не является расчётной книгой ОБМ.
